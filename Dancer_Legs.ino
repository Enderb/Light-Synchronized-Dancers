...
#define topic_name_pub "Dancer_Legs"
#define topic_name_sub "Dancer_Arms"
char espUUID[12] = "Dancer_Legs";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (r == 3) {
    colorWipe_right(strip_right.Color(255, 255, 255), 0); // White
  }
  ...
  if (l == 3) {
    colorWipe_left(strip_left.Color(255, 255, 255), 0); // White
  }
...
void setup(void) {
  ...
  if (! lis_right.begin(0x18)) {   // change this to 0x19 for alternative i2c address
...
void loop() {
  ...
  Serial.print("Rigth X:  "); Serial.print(lis_right.x); 
